#!/usr/bin/env bash
# Roda as sete baterias em sequência e devolve 0 só se todas passarem.
...
echo "═══ 1/7  funcional (escrita de cada módulo pela API)"
./testes/funcional.sh || FALHOU=1

echo
echo "═══ 2/7  sistema (16 seções, desktop e celular)"
node testes/sistema.js || FALHOU=1

echo
echo "═══ 3/7  participante (tela pública, celular)"
...
echo
echo "═══ 4/7  backup (gerar, verificar, restaurar)"
...
echo
echo "═══ 5/7  e-mail (envio por API, com serviço de mentira)"
...
echo
echo "═══ 6/7  backup remoto (cópia fora do provedor, com serviço de mentira)"
...
echo
echo "═══ 7/7  limpar plano de ação (cli/limpar_plano_acao.php)"
